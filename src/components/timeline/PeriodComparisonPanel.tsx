  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div>
          <label className="block text-[12px] font-medium text-charcoal-400 uppercase tracking-wider mb-2" htmlFor="period-a">
            Earlier period
          </label>
          <select
            id="period-a"
            ...
            className="w-full bg-white border border-cream-200 rounded-lg px-4 py-2.5 text-[14px] text-charcoal-700 focus:outline-none focus:border-accent-500"
          >
            {periods.map((p, i) => (
        ...
        <div>
          <label className="block text-[12px] font-medium text-charcoal-400 uppercase tracking-wider mb-2" htmlFor="period-b">
            Later period
          </label>
          <select
            id="period-b"
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div className="card p-4">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-[20px] font-semibold text-charcoal-700">{comparison.postingFrequency.value.a}</span>
              <span className="text-charcoal-300 text-[13px]">&rarr;</span>
              <span className="text-[20px] font-semibold text-charcoal-900">{comparison.postingFrequency.value.b}</span>
            </div>
          </div>
          ...
          <div className="card p-4">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-[20px] font-semibold text-charcoal-700">{comparison.topicDiversity.value.a}</span>
              <span className="text-charcoal-300 text-[13px]">&rarr;</span>
              <span className="text-[20px] font-semibold text-charcoal-900">{comparison.topicDiversity.value.b}</span>
            </div>
          </div>
            ...
            <div className="flex gap-2">
              <div className="flex flex-wrap gap-1">
                {comparison.topTopics.value.a.map(t => (
                  <span key={t} className="pill text-[10px] bg-cream-200 text-charcoal-600">{t}</span>
                ))}
              </div>
              <span className="text-charcoal-300 text-[13px]">&rarr;</span>
              <div className="flex flex-wrap gap-1">
          ...
          {Object.entries(comparison.toneShift.value.a).map(([k, v]) => (
            <div key={k} className="card p-4">
              <p className="text-[11px] font-medium text-charcoal-400 uppercase tracking-wider mb-2">{k}</p>
              <div className="flex items-baseline gap-2">
                <span className="font-mono text-[16px] text-charcoal-700">{v.toFixed(2)}</span>
                <span className="text-charcoal-300 text-[12px]">&rarr;</span>
                <span className="font-mono text-[16px] text-charcoal-900">{(comparison.toneShift.value.b as Record<string, number>)[k].toFixed(2)}</span>
              </div>
            </div>
          ))}
        </div>
