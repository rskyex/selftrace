...
export function InsightCard({ label, value, detail, status, trend, className = '' }: InsightCardProps) {
  return (
    <div className={`card p-5 ${className}`}>
      <p className="text-[12px] font-medium text-ink-400 uppercase tracking-wider mb-2">
        {label}
      </p>
      <div className="flex items-baseline gap-2">
        <p className="text-[28px] font-semibold text-ink-900 tracking-tight leading-none">
          {value}
        </p>
        {trend && (
          <span className={`text-[13px] font-medium ${
            trend === 'up' ? 'text-accent-600' : trend === 'down' ? 'text-amber-600' : 'text-ink-400'
