...
export function AreaChart({ data, caption, confidence, height = 180, color = 'coral' }: AreaChartProps) {
  ...
  const c = { coral: { s: '#E07A5F', f: '#FFF7F2' }, sage: { s: '#7BAE8B', f: '#E8F3EC' }, gold: { s: '#D4A87C', f: '#FAF0E6' }, ink: { s: '#6B635B', f: '#F4F2ED' } };
    ...
    <div className="my-4">
      <div className="surface-quiet">
        <svg viewBox={`0 0 ${w} ${height}`} className="w-full" style={{ maxHeight: `${height + 12}px` }} role="img" aria-label={caption || 'Chart'}>
          {[0, 0.5, 1].map(f => <line key={f} x1={pad.l} x2={w - pad.r} y1={pad.t + cH * (1 - f)} y2={pad.t + cH * (1 - f)} stroke="#EDEAE4" strokeWidth={1} />)}
          <path d={area} fill={c[color].f} opacity={0.8} />
          <path d={line} fill="none" stroke={c[color].s} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />
          {data.map((d, i) => {
            if (i % step !== 0 && i !== data.length - 1) return null;
            const dt = new Date(d.date);
            return <text key={i} x={x(i)} y={height - 4} textAnchor="middle" fill="#9B958E" fontSize={11} aria-hidden="true">{`${dt.toLocaleString('en', { month: 'short' })} '${dt.getFullYear().toString().slice(2)}`}</text>;
          })}
        </svg>
      </div>
      {(caption || confidence) && (
