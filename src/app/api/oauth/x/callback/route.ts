...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(`${appUrl}/social?error=${encodeURIComponent(error)}`);
  }
  ...
  try {
    ...
    if (dbError) {
      ...
      return NextResponse.redirect(`${appUrl}/social?error=save_failed`);
    }

    return NextResponse.redirect(`${appUrl}/social?connected=x&username=${profile.username}`);
  } catch (err) {
    console.error('X OAuth callback error:', err);
    const message = err instanceof Error ? err.message : 'unknown_error';
    return NextResponse.redirect(`${appUrl}/social?error=${encodeURIComponent(message)}`);
  }
}
