...
function ConnectInner() {
  ...
  useEffect(() => {
    ...
    if (error) {
      setSocialMessage({ type: 'error', text: decodeOAuthError(error) });
    } else if (connected && username) {
      setSocialMessage({ type: 'success', text: `Connected @${username} on ${connected.toUpperCase()}.` });
    }
  }, [searchParams]);
  ...
  async function handleImport(accountId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        setSocialMessage({ type: 'error', text: data.error ?? 'Import failed.' });
      } else if (data.imported === 0) {
        setSocialMessage({ type: 'error', text: 'No posts found on this account.' });
      } else {
        setSocialMessage({ type: 'success', text: `Imported ${data.imported} posts.` });
      }
    } catch {
      setSocialMessage({ type: 'error', text: 'Network error during import.' });
    } finally {
      setImporting(null);
    }
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Connect an account, upload an export, or try a demo profile to see your patterns."
      />
      ...
      <div className="wide-column px-6 pb-24">
        ...
        {/* Connect account + Demo — primary grid */}
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          {/* Connect account card (replaces upload card) */}
          <div className="card p-8">
            <h2 className="font-display text-[20px] text-ink-900 mb-3">
              Connect an account
            </h2>
            <p className="text-[14px] text-ink-500 leading-relaxed mb-6">
              Link your social media account via official login, import your
              posts, and run SelfTrace analysis on your own data.
            </p>
